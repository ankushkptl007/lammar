    ...
    a.ThreeSixty = function (b, c) {
        ...
        e.$el = a(b), e.el = b, e.$el.data("ThreeSixty", e), e.init = function () {
            ...
            var c = e.el.offset(),
                ...
                h = d - f,
                i = g - h;
            return 0 !== f && "undefined" !== d || (i = "undefined"), i
        }, e.resize = function () {
            var b = d.width,
                c = d.height,
                f = c / b,
                g = b / c,
                h = e.el.parent().width(),
                i = e.el.parent().height(),
                j = e.maxHeight(),
                k = a(window).width(),
                l = a(window).height(),
                m = h * f;
            h > k && (h = k), i > l && (i = l), 0 !== j && "undefined" !== j || (j = l), h * f > j ? e.$el.css({
                height: j + "px",
                width: j * g + "px"
            }) : e.$el.css({
                width: h + "px",
        ...
        }, e.fSBackgroundColor = function () {
            var a = d.fSBackgroundColor;
            return a
            ...
            var b = d.width,
                ...
                h = screen.height,
                i = screen.width,
                j = e.fSBackgroundColor;
            if (d.fullscreen) {
                var k = e.$el[0];
                screenfull.enabled && (screenfull.toggle(k), screenfull.onchange(function () {
                    screenfull.isFullscreen ? (a(".btnFull span").removeClass("icon-fullscreen").addClass("icon-resize"), i * f > h ? e.$el.css({
                        height: h + "px",
                        width: h * g + "px",
                        backgroundColor: j
                    }) : e.$el.css({
                        ...
                        height: i * f + "px",
                        backgroundColor: j
                    })) : (a(".btnFull span").removeClass("icon-resize").addClass("icon-fullscreen"), e.$el.css({
                        width: b + "px",
                        height: c + "px",
                        backgroundColor: ""
                    }), e.resize())
            ...
            }), d.styles && e.$el.css(d.styles), e.resize(), e.$el.find(d.progress).css({
                marginTop: d.height / 2 - 15 + "px"
            }), e.$el.find(d.progress).fadeIn("slow"), e.$el.find(d.imgList).hide()
        }, e.loadImages = function () {
            var b, c, g, h;
            b = document.createElement("li"), h = d.zeroBased ? 0 : 1, c = d.imgArray ? d.imgArray[d.loadedImages] : d.domain + d.imagePath + d.filePrefix + e.zeroPad(d.loadedImages + h) + d.ext + (e.browser.isIE() ? "?" + (new Date).getTime() : ""), g = a("<img>").attr("src", c).attr("alt", "360 Slider Images").addClass("previous-image normal").appendTo(b), f.push(g), e.$el.find(d.imgList).append(b), e.imageLoadNext()
        }, e.imageLoadNext = function () {
            d.loadedImages += 1, d.loadedImages >= d.totalFrames ? e.imagesLoaded() : e.loadImages()
        }, e.imagesLoaded = function () {
            var b = 0;
            a.each(f, function (c, g) {
                a(g).on("load", function () {
                    b += 1, a(d.progress + " span").text(Math.floor(b / d.totalFrames * 100) + "%"), b >= d.totalFrames && (d.disableSpin && f[0].removeClass("previous-image").addClass("current-image"), a(d.progress).fadeOut("slow", function () {
            ...
            if (d.navigation && !d.navigation_init) {
                var b, c, f, g, h, i, j, k, l, m;
                d.position ? (m = d.position, b = a("<div/>").attr("class", "nav_bar " + m)) : b = a("<div/").attr("class", "nav_bar top-right"), g = a("<div/>").attr("class", "btnPrev butn"), h = a("<span/>").attr("class", "icon-back"), i = a("<div/>").attr("class", "btnPlay butn"), j = a("<span/>").attr("class", "icon-play"), c = a("<div/>").attr("class", "btnNext butn"), f = a("<span/>").attr("class", "icon-forward"), g.append(h), i.append(j), c.append(f), b.append(g), b.append(i), b.append(c), d.fullscreen && (k = a("<div/>").attr("class", "btnFull butn"), l = a("<span/>").attr("class", "icon-fullscreen"), k.append(l), b.append(k)), e.$el.prepend(b), c.bind("mousedown touchstart", e.next), g.bind("mousedown touchstart", e.previous), i.bind("mousedown touchstart", e.play_stop), d.fullscreen && k.bind("mousedown touchstart", e.fullscreen), d.navigation_init = !0
            }
        }, e.play_stop = function (b) {
            b.preventDefault(), d.autoplay ? (d.autoplay = !1, a(".btnPlay span").removeClass("icon-pause").addClass("icon-play"), clearInterval(d.play), d.play = null) : (d.autoplay = !0, d.play = setInterval(e.moveToNextFrame, d.playSpeed), a(".btnPlay span").removeClass("icon-play").addClass("icon-pause"))
        }, e.next = function (a) {
            a && a.preventDefault(), 1 === d.autoplayDirection ? d.endFrame -= 5 : d.endFrame += 5, e.refresh()
            ...
            var a, b;
            return d.disableWrap ? (a = Math.min(d.currentFrame, d.totalFrames - (d.zeroBased ? 1 : 0)), b = Math.min(d.endFrame, d.totalFrames - (d.zeroBased ? 1 : 0)), a = Math.max(a, d.zeroBased ? 0 : 1), b = Math.max(b, d.zeroBased ? 0 : 1), d.currentFrame = a, d.endFrame = b) : (a = Math.ceil(d.currentFrame % d.totalFrames), a < 0 && (a += d.totalFrames - (d.zeroBased ? 1 : 0))), a
